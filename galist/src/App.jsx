...
function App() {
              ...
              setTimeout(() => {
                setCircles((prevCircles) => {
                  ...
                  if (newCircles.length === 0 && currentExercise) {
                    console.log(
                      "All circles entered portal - validating submission..."
                    );

                    // Ensure exercise is loaded
                    if (!exerciseManagerRef.current.currentExercise) {
                      console.warn(
                        "No exercise loaded, loading basic exercise..."
                      );
                    ...
                    setTimeout(() => {
                      try {
                        ...
                        console.log(
                          ...
                          exerciseManagerRef.current.currentExercise
                        );
                        console.log("Submission data:", submissionData);
                        console.log(
                          "Original circles length:",
                          submissionData.circles.length
                        );
                        console.log(
                          "Current connections length:",
                          submissionData.connections.length
                        );
                        console.log("Entry order:", finalEntryOrder);
                        console.log(
                          "About to call validateSubmission with parameters:",
                          {
                        ...
                        if (
                          ...
                          console.warn(
                            "Non-critical validation error, continuing:",
                            error
                          );
                        }
                      }
  ...
  const handleSpecificInsertion = (index) => {
    ...
    if (headNodes.length === 0) {
      ...
      if (targetIndex >= chainOrder.length) {
        ...
        console.log(
          `Inserted at index ${targetIndex} between nodes:`,
          prevNodeId,
          "and",
          nextNodeId
        );
  ...
  const handleDeleteCircle = () => {
    ...
    setConnections((prevConnections) => {
      ...
      if (isMiddle) {
        ...
        console.log(
          `Middle node deleted: Created ${newConnections.length} bridge connections`
        );
        ...
        console.log(
          "Head node deleted: Next nodes become new heads:",
          newHeadIds
        );
        ...
        console.log(
          "Tail node deleted: Previous nodes become new tails:",
          newTailIds
        );
      } else if (
        ...
        console.log(
          "Start node deleted: Next nodes become new starting points"
        );
      } else if (isIsolated) {
    ...
    console.log(
      `Node ${nodeToDelete} successfully deleted with connection adjustments`
    );
  };
  ...
  return (
    <div className="app">
      {/* Video background (hidden while menu is open) */}
      {/* Video background (only during gameplay) */}
      {selectedMode && (
        <video
      ...
      {/* Mode Select Overlay (after Start) */}
      {!showMenu && !selectedMode && <ModeSelect onSelect={handleSelectMode} />}

      {/* Instruction button in top left */}
      {/* Instruction button in top left (hidden until mode selected) */}
      ...
      {/* Portal and right square (hidden while menu is open) */}
      {!showMenu && (
        <>
          {/* Portal and right square (hidden until mode selected) */}
          {selectedMode && (
            <>
              <PortalComponent
      ...
      {/* Input controls (hidden until mode selected) */}
      {!showMenu && selectedMode && (
        <div className="controls">
          ...
          <button
            ...
            className={`portal-button ${
              !isPortalButtonEnabled ? "disabled" : ""
            } ${isPortalOpen ? "open" : ""}`}
      ...
      {/* Animated circles (hidden until mode selected) */}
      {!showMenu &&
        selectedMode &&
        circles.map((circle) => {
          const isHead = isHeadNode(circle.id);
          ...
          return (
            <div
              ...
              className={`animated-circle ${
                suckingCircles.includes(circle.id) ? "being-sucked" : ""
              }`}
              style={{
                left: `${circle.x - 30}px`,
                top: `${circle.y - 30}px`,
      ...
      {/* Connection lines (hidden until mode selected) */}
      {!showMenu && selectedMode && (
                            ...
                            >
                              ...
                              {index <
                                currentExercise.expectedStructure.length -
                                  1 && <td className="arrow-cell-empty"></td>}
                            </React.Fragment>
                          )
                        )}
                      </tr>
                                ...
                                >
                                  <td className="user-cell">
                                    {userNode ? (
                                      ...
                                      >
                                        <div className="user-node-value">
                                          {userNode.value}
                                        </div>
                                        <div className="user-node-address">
                                          {userNode.address}
                                        </div>
                                      </div>
                                    ) : (
        ...
        <div className="popup-overlay" onClick={closePopup}>
          <div className="popup-content" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="popup-circle">
              ...
              <span className="popup-circle-address">
            ...
            <div className="popup-form-container">
              ...
              <div className="popup-buttons">
                <button
                  ...
                  className="popup-button connect-btn"
                >
                  CONNECT
                </button>
                <button
      ...
      {showInsertModal && (
        <div className="insert-modal-overlay" onClick={closeInsertModal}>
          <div
            className="insert-modal-content"
            onClick={(e) => e.stopPropagation()}
          >
            {/* X button to close modal */}
            <button
              className="insert-modal-close-btn"
      ...
      {showIndexModal && (
        <div className="index-modal-overlay" onClick={closeIndexModal}>
          <div
            className="index-modal-content"
            onClick={(e) => e.stopPropagation()}
          >
            {/* X button to close modal */}
            <button className="index-modal-close-btn" onClick={closeIndexModal}>
              ×
            </button>
            ...
            <div className="index-input-container">
              <input
                ...
                autoFocus
              />
              <button onClick={handleIndexSubmit} className="index-go-btn">
                Go
              </button>
            </div>
          </div>
